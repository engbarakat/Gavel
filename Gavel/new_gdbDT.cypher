...
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/switchesDT.csv" AS row
CREATE (:Switch {id: row.id, layer: toInt(row.layer), dpid: row.dpid});

USING PERIODIC COMMIT
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/hostsDT.csv" AS row
...
USING PERIODIC COMMIT
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/connected_toDT.csv" AS row
...
USING PERIODIC COMMIT
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/switched_toDT.csv" AS row
