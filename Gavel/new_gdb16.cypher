...
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/switches16.csv" AS row
CREATE (:Switch {id: row.id, layer: toInt(row.layer), dpid: row.dpid});

USING PERIODIC COMMIT
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/hosts16.csv" AS row
...
USING PERIODIC COMMIT
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/connected_to16.csv" AS row
...
USING PERIODIC COMMIT
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/switched_to16.csv" AS row
