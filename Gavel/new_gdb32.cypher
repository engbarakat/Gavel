...
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/switches32.csv" AS row
CREATE (:Switch {id: row.id, layer: toInt(row.layer), dpid: row.dpid});

USING PERIODIC COMMIT 10000
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/hosts32.csv" AS row
...
USING PERIODIC COMMIT 10000
LOAD CSV WITH HEADERS FROM "file:///TestedTopologies/connected_to32.csv" AS row
