...
LOAD CSV WITH HEADERS FROM "file:///switches64.csv" AS row
CREATE (:Switch {id: row.id, layer: toInt(row.layer), dpid: row.dpid});

USING PERIODIC COMMIT 10000
LOAD CSV WITH HEADERS FROM "file:///hosts64.csv" AS row
...
USING PERIODIC COMMIT 10000
LOAD CSV WITH HEADERS FROM "file:///connected_to64.csv" AS row
...
USING PERIODIC COMMIT 10000
LOAD CSV WITH HEADERS FROM "file:///switched_to64.csv" AS row
